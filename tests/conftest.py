...
"""
    ...
    subprocess.run(["git", "commit", "-m", "Add Python project structure"], cwd=temp_git_repo, capture_output=True)
...
def node_project(temp_git_repo: Path) -> Path:
    ...
    # Commit changes
    subprocess.run(["git", "add", "."], cwd=temp_git_repo, capture_output=True)
    subprocess.run(["git", "commit", "-m", "Add Node.js project structure"], cwd=temp_git_repo, capture_output=True)
...
"""
    ...
    # Commit changes
    subprocess.run(["git", "add", "."], cwd=temp_git_repo, capture_output=True)
    subprocess.run(["git", "commit", "-m", "Add Docker configuration"], cwd=temp_git_repo, capture_output=True)
...
def spec_with_plan(spec_dir: Path) -> Path:
    ...
    plan_file = spec_dir / "implementation_plan.json"
    with open(plan_file, "w") as file_handle:
...
def make_commit(temp_git_repo: Path):
    ...
    def _make_commit(filename: str, content: str, message: str) -> str:
        ...
        subprocess.run(["git", "add", "."], cwd=temp_git_repo, capture_output=True)
        subprocess.run(["git", "commit", "-m", message], cwd=temp_git_repo, capture_output=True)
...
def temp_project(temp_git_repo: Path):
    ...
    # Commit the files
    subprocess.run(["git", "add", "."], cwd=temp_git_repo, capture_output=True)
    subprocess.run(["git", "commit", "-m", "Add source files"], cwd=temp_git_repo, capture_output=True)
